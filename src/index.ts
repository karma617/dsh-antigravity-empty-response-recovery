      ...
      try {
        ...
        for await (const chunk of stream) {
          ...
          if (streaming) {
            ...
            if (chunk.type === 'text-delta' && chunk.text) totalTextChars += chunk.text.length
            if (chunk.type === 'tool-call-delta' && (chunk.name || chunk.argumentsDelta || chunk.id)) totalToolCalls++
            if (chunk.type === 'reasoning-delta' && chunk.text) totalReasoningChars += chunk.text.length
          ...
          if (chunk.type === 'block-end') {
            ...
            if (b?.type === 'text' && b.text) {
              ...
              if (!accumulatedText) accumulatedText = b.text
            }
            if (b?.type === 'tool-call') {
              isContentChunk = true
              totalToolCalls++
          ...
          if (isContentChunk) {
            ...
            const looksLikeLeakedCall = totalToolCalls === 0 && (
      ...
      if (config.retryWithNudge) {
        ...
        try {
          ...
          if (origMessages.length > 0) {
            const lastMsg: any = origMessages[origMessages.length - 1]
